...
export function meetsContrastRequirement(
  ...
  if (!passes) {
    ...
    recommendation =
      `Contrast ratio is ${ratio.toFixed(2)}:1, needs to be at least ${required}:1. ` +
...
export function verifyThemeContrast(
  ...
  checks.push(meetsContrastRequirement(colors.text, colors.background, level, 'normal'));

  // Secondary text on background
  checks.push(meetsContrastRequirement(colors.textSecondary, colors.background, level, 'normal'));

  // Primary text on surface
  checks.push(meetsContrastRequirement(colors.text, colors.surface, level, 'normal'));

  // Secondary text on surface
  checks.push(meetsContrastRequirement(colors.textSecondary, colors.surface, level, 'normal'));

  // Primary color on background (for buttons, links)
  checks.push(meetsContrastRequirement(colors.primary, colors.background, level, 'large'));

  // Border on background
  checks.push(meetsContrastRequirement(colors.border, colors.background, level, 'large'));

  // Link on background
  checks.push(meetsContrastRequirement(colors.link, colors.background, level, 'normal'));

  // Semantic colors on background
  checks.push(meetsContrastRequirement(colors.success, colors.background, level, 'normal'));
  checks.push(meetsContrastRequirement(colors.warning, colors.background, level, 'normal'));
  checks.push(meetsContrastRequirement(colors.error, colors.background, level, 'normal'));
  checks.push(meetsContrastRequirement(colors.info, colors.background, level, 'normal'));
