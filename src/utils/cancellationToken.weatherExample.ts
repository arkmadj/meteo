...
export class EnhancedWeatherService {
  ...
  async fetchWeatherData(
    ...
    const compositeToken = timeoutToken 
      ? anyToken(operationToken, timeoutToken)
      : operationToken;
  ...
  async fetchBatchWeatherData(
    ...
    const compositeToken = timeoutToken 
      ? anyToken(batchToken, timeoutToken)
      : batchToken;
    ...
    if (errors.size > 0 && results.size === 0) {
      // All requests failed
      throw new Error(`All weather requests failed: ${Array.from(errors.values()).map(e => e.message).join(', ')}`);
    }
